...
| 08 | [x] | `hotel/08-Product-Cards.xlsx` (+ `08-Product-Group-List.xlsx`) | Hotel wizard SELLABLE | — |
| 09 | [x] | `hotel/09-Stock-Cards.xlsx` | Hotel wizard STOCK | — |
...
| 15 | [x] | `hotel/15-Hizmet-Tanimlari.xlsx` (287) | Clinic catalog + Hotel `SPA MEDIKAL` | — |
...
| 44 | [ ] | `1c/44-1C-Counterparties.xlsx` | Finance | **Справочник «Контрагенты».** Полное и краткое имя, ВÖЕН (10 цифр), банковские реквизиты (IBAN, банк), признак (поставщик / юрлицо / OTA-агент / физлицо), телефон, договоры (номер, дата, валюта). Все, с кем есть оборот или остаток на дату среза. |
...
| 53 | [ ] | `1c/53-1C-Procedure-Consumables.xlsx` | Clinic / Retail | **Нормы списания на процедуру.** Процедура (имя как в медкаталоге) → код ТМЦ из аптеки/склада → количество на 1 сеанс. Если в 1С норм нет — написать «нет». |
