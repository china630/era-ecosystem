...
| Room plan bars | Half-day model; blunt EW chevron (`CHEVRON_PX`) whenever checkout is in-window; matching left butt notch on in-window starts; flat = clipped; day-state palette + HK squares (`plan-bar-theme.ts`); share N lanes without overlay (`share-lanes.ts`) | C+ / EW palette |
