...
export default function YieldRulesPage() {
  ...
  const [loadBasedOn, setLoadBasedOn] = useState<boolean | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [rulesData, policyData] = await Promise.all([rulesRes.json(), policyRes.json()]);
      if (!rulesRes.ok) {
        showApiError(rulesData, t('loadFailed'));
        return;
      }
      setRules(Array.isArray(rulesData) ? rulesData : []);
      if (policyRes.ok) {
        setLoadBasedOn(Boolean(policyData.loadBasedPricingEnabled));
      }
    } catch (e) {
  ...
  const filteredRules = useMemo(() => {
    ...
    return rules.filter((r) => {
      ...
      if (needle && !r.propertyCode.toLowerCase().includes(needle)) return false;
      return true;
    });
  }, [rules, debouncedQ, activeFilter]);
  ...
  async function saveRule(e: React.FormEvent) {
    ...
    if (!form.propertyCode.trim()) {
      showApiError({ error: t('propertyRequired') });
      return;
    }
    setBusy(true);
    try {
      const payload = {
        propertyCode: form.propertyCode.trim().toUpperCase() || 'DEFAULT',
  ...
  return (
    <>
      ...
      {loadBasedOn != null && (
        ...
        >
          {loadBasedOn ? t('loadBasedOn') : t('loadBasedOff')}
        </p>
      )}
              ...
              {filteredRules.map((rule) => (
                <tr key={rule.id} className={DATA_TABLE_TR_CLASS}>
                  ...
                  <td className={DATA_TABLE_TD_CLASS}>
                    ...
                    >
                      {rule.active ? t('activeBadge') : t('inactiveBadge')}
                    </span>
                  </td>
                  <td className={DATA_TABLE_TD_CLASS}>
