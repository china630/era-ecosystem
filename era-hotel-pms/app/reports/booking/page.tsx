...
export default function BookingHubPage() {
  ...
  const reports = getReportsByCategory('booking');

  return (
    <div className="mx-auto max-w-5xl p-6">
      <h1 className="mb-6 text-2xl font-semibold">{t('reports.booking')}</h1>
      {reports.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {reports.map((r) => (
            ...
            >
              <h2 className="font-medium">{t(r.titleKey as 'nav.guestDedup')}</h2>
            </Link>
          ))}
        </div>
      )}
