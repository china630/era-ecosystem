...
export default function AgencyHubPage() {
  ...
  const reports = getReportsByCategory('agency');

  return (
    <div className="mx-auto max-w-5xl p-6">
      <h1 className="mb-6 text-2xl font-semibold">{t('reports.agency')}</h1>
      {reports.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {reports.map((r) => (
            ...
            >
              <h2 className="font-medium">{t(r.titleKey as 'reports.agencyProfitTitle')}</h2>
            </Link>
          ))}
        </div>
      )}
