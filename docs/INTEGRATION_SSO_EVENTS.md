...
**Entitlement fail-closed (Wave 9):** satellites use `*-module-gate.ts` / `requireSatelliteModule` — inactive module → 403 (ops) or cron skip. Kit `ERA_DEV_UNLOCK_ALL_MODULES=1` is ignored when `NODE_ENV=production` (prod refuses DEV unlock). **Request org first:** hotel/clinic enter ALS from session / `x-era-organization-id` before the gate; kit then `assertEntitled` via CP snapshot (`ORCHESTRATOR_EVENT_URL` + service token). Sync runtime-config `activeModules` is cache when CP is unreachable — not the SHARED SoR. Spot-check: retail + CRM main ops routes call `assert*Entitled` (enter tenant, then gate). AC entitlement notes stay 🟡 (not Scaffold ✅).
