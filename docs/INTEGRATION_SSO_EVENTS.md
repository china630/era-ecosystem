...
- **Exchange:** satellite `POST /api/auth/sso/exchange` via `resolveVerifiedSsoFinanceRole` + `consumeSsoSignatureOnce` (SEC-SSO-01 process-local replay guard; Redis planned for multi-instance). When deployment org is bound (`ERA_SATELLITE_ORGANIZATION_ID` or runtime bind via `satelliteOrganizationId()`), ticket org must match (SEC-SSO-05) on DEDICATED/ONPREM; SHARED accepts ticket org. See ADR [`satellite-organization-bind.md`](adr/satellite-organization-bind.md). Ticket `organizationId` remains **UUID**. Local staff `/login` on SHARED (industry **and** `era-bank` ops) uses public `orgNo` (not UUID) — [`org-public-number-and-login-host.md`](adr/org-public-number-and-login-host.md). Finance/orch login do not use `orgNo`.
...
### Buyer portal SSO (Finance trade credit)

- **ADR:** [`finance-trade-credit-control.md`](adr/finance-trade-credit-control.md) §7.1. Identity SoR = orchestrator `BuyerPortalAccount` + `BuyerOrgGrant` (not staff membership, not hotel agency).
- **Mint:** orchestrator after buyer password login + org pick (`POST /buyer-portal/login`, `POST /buyer-portal/orgs/pick`) — HMAC with `ERA_SSO_SHARED_SECRET`.
- **Payload:** `buyer|{email}|{organizationId}|{counterpartyId}|{expiresAt}` (+ `|jti`). Distinct from owner/staff satellite launch and hotel agency SSO.
- **Exchange:** Finance `POST /api/auth/buyer-sso/exchange` → cookie `era_buyer_session`. Must **not** map into Finance OWNER/ADMIN/ACCOUNTANT or wholesale sales roles.
- **Scope:** Finance `/buyer/*` and buyer-scoped APIs only. UI entry: Orch `/buyer/login` → Finance launch URL.
...
**Desired-state runtime config (industry API — Wave 2+6+7):** same Sync also POSTs `POST /api/internal/v1/runtime-config` (event URL, public base URL, PSA emails, SSO shared secret ≥16, event token, optional `vendorBridgesEnabled`, `activeModules` / optional `hotelModules`, optional `deploymentTopology` + `edition`). Receivers: hotel/clinic/fnb + thin industry (retail/crm/auto/construction/wholesale/logistics) + bank/dbo + Finance Nest + bank-core Nest (absolute `/api/internal/v1/*`, excluded from `/api/v1` prefix). Kit store: `_era_runtime_config` + `.data/runtime-config.json`; env = bootstrap override. **Boot:** file cache then DB (DB wins), then optional **pull** `GET /v1/internal/satellites/desired-state?satelliteKey=` (Bearer + `X-Organization-Id`; enabled `SatelliteEndpoint` required; folklore omitted — same builder as Sync). Default pull on when `ERA_IN_DOCKER=1` (`ERA_DESIRED_STATE_PULL=0|1` override); orch down keeps local snapshot (boot does not fail). **Reconcile (Wave 7):** jittered `ERA_DESIRED_STATE_POLL_MS` (default 60s; `0` = off); hash skip; 401 stops loop; process-wide fields only (per-org vendor bridges stay Sync). **Not** PlacementJob. **Folklore secrets** (`dev-satellite-event-token`, `dev-control-plane-token`, `change-me…`) are never written by Sync/pull, never stomp Docker install env, and omitted Sync fields do not wipe an existing real token. In Docker, `orchestratorEventUrl` is `ERA_ORCHESTRATOR_INTERNAL_URL` / `http://orchestrator:4000`, not public Traefik HTTPS (including a mis-set internal URL). Event publish **throws** on 401/403 (`SatelliteEventAuthError`) so a stale token cannot silently freeze hotel→clinic. Staff fan-out uses `CLINIC_API_URL` (`http://clinic:3203`) when the registry launch URL is public HTTPS or loopback. Finance Nest resolves orch URL via `@era/satellite-kit` `resolveOrchestratorBaseUrl()` (memory first; `CONTROL_PLANE_URL` install bootstrap). `deploymentTopology` is informational — never skip tenant filter. Boot: Next `instrumentation.ts` / Nest bootstrap via `onSatelliteBoot`. **Deny:** missing Bearer → **401**; `ssoSharedSecret` shorter than 16 → **400** (Zod / class-validator); orch Sync/pull omits secrets `<16` and folklore placeholders rather than pushing them.

**Entitlement fail-closed (Wave 9):** satellites use `*-module-gate.ts` / `requireSatelliteModule` — inactive module → 403 (ops) or cron skip. Kit `ERA_DEV_UNLOCK_ALL_MODULES=1` is ignored when `NODE_ENV=production` (prod refuses DEV unlock). **Request org as an argument** (session / `x-era-organization-id`) — not ALS `enterWith` across Next `await`. Kit `assertEntitled` via CP snapshot; `GET /internal/v1/subscription/snapshot` accepts `SATELLITE_EVENT_SERVICE_TOKEN` as well as the control-plane token. Host `npm run dev` rewrites Compose DNS (`http://orchestrator:4000`) to `http://127.0.0.1:4000` so entitlement snapshot works outside Docker. Compose images set `ERA_IN_DOCKER=1` and `NODE_ENV=production` so that rewrite **does not** run in the clinic container (loopback inside the container is not the orchestrator). Sync runtime-config `activeModules` is cache when CP is unreachable. Spot-check: retail + CRM `assert*Entitled` (enter tenant, then gate with that org). AC entitlement notes stay 🟡 (not Scaffold ✅).
...
| `WORKFORCE_PERSONNEL_ORDER_ISSUED` | CP audit + optional future satellite event. Wave 4: ISSUED freezes `contextJson` (leave remaining via Finance S2S). See [evrostar-wave-4.md](./runbooks/evrostar-wave-4.md). |
...
**Finance S2S (CP-WF-MIG-01 opening):** `POST /internal/v1/workforce/employees/opening` (same token, `hr_full`). Body: `organizationId`, `cpEmploymentId`, optional `salary`, `internalRate`, `balanceDays`, `baseVacationDaysPerYear`, `asOfDate`. Vacation `initialVacationDays = balanceDays − accrued(hire → asOf)` so the as-of balance matches the file. Leave history is not mirrored; do not emit `WORKFORCE_ABSENCE_APPROVED`.
...
**Workforce roster CSV (orch import):** canonical headers `fin,firstName,middleName,lastName,sex,birthDate,orgUnit,position,hireDate,workplace,satellites`. Prefer explicit name parts — a bare `fullName` blob is ambiguous for 2-token AZ surname-first. Legacy `fullName` is still accepted: MDM `splitFullNameToParts` uses given-first by default, and treats **≥3 tokens ending in** `oğlu`/`qızı` (and latin/cyrillic variants) as **AZ surname-first** (`Soyad Ad Ata`). Repair scrambled cipher parts: `FORCE=1` on `packages/mdm-database/prisma/scripts/backfill-person-name-parts.ts`. Nafta HR cutover `Tam adı` (surname-first) is parsed in `era-clinic/scripts/nafta-cutover/map.cjs` → three columns before orch import. Export roster adds parts + `displayName`.
...
**Satellite login hardening (2026-09):** CP stores `satelliteStaffLogin` / `satelliteStaffPin` on `WorkforceEmployment`. Partial unique index: one login per org among **ACTIVE** employments. `PATCH …/reprovision` and hire reject `LOGIN_TAKEN` (409) on collision and `LOGIN_REQUIRES_BINDING` when login/pin is sent without an active role binding **and** `satelliteKeys` is omitted. Sending `satelliteKeys: []` (or a set with no remaining keys) revokes access without that error; login/PIN is not saved. Workspace **Login & access** is enabled for any non-terminated employment. Satellite handlers (`era-hotel-pms`, `era-clinic`, `era-fnb-pos` `/api/integration/staff-provision`) resolve user by `cpEmploymentId` first; login fallback never steals another employment's row — collision → **409** `{ code: "LOGIN_TAKEN" }` (also Prisma unique `P2002` on `User.login`). **STAFF_DEACTIVATED** disables User by `satelliteUserId`, else exactly one User by `{ organizationId, cpEmploymentId }` (0 → idempotent OK; >1 → **409** `TARGET_AMBIGUOUS`). Clinic also sets `Practitioner.active=false` where `userId` matches; F&B sets `StaffRoster.active=false` for the unique `cpEmploymentId` row. Excel-only practitioners (`userId` null) stay untouched. Binding `provisionState` (`PENDING`/`APPLIED`/`FAILED`) + `lastProvisionError` updated by fan-out worker; UI badge + Reprovision = Retry. Fan-out worker marks `FAILED` on bridge errors (`LOGIN_TAKEN` when bridge returns **409** with that code). List SoR: `GET …/security/bindings` (includes MDM `persons`), paginated `manual-grants` / `employments` — not overview tails. ADR: [cp-workforce-satellite-provision-sync.md](./adr/cp-workforce-satellite-provision-sync.md), [cp-workforce-role-templates-and-security-admin.md](./adr/cp-workforce-role-templates-and-security-admin.md).

**Reprovision UI:** Workspace → Workforce → Employments → row **⋯** → Reprovision (confirm) re-emits current bindings (disabled until at least one active binding). **Login & access** (same overflow) edits **per-person** Hotel / Clinic / F&B checkboxes — including hire-default bindings, not only `MANUAL_GRANT`. Save sends `PATCH …/reprovision` with `satelliteKeys` (empty array = revoke all satellites for this employment). Role matrix remains position defaults; Bindings remains a journal. `GET /platform/v1/workforce/employments` and employment detail include those bindings. Creating a manual grant also calls the same `reprovision` internally.
...
**CP workforce org events (Plan B / Evrostar wave 0):** Orchestrator → `era-satellite-events` → Finance worker when `hr_full`:

| Event | Purpose |
|-------|---------|
| `WORKFORCE_ORG_UNIT_UPSERTED` | Upsert Finance `Department` CostCenter mirror (`cpOrgUnitId`). Ensures Finance `Organization` UUID exists first. Missing parent dept → throw/retry (not silent skip). |
| `WORKFORCE_ORG_UNIT_ARCHIVED` | Soft-delete Department mirror |
| `WORKFORCE_POSITION_UPSERTED` | Upsert Finance `JobPosition` mirror (`cpPositionId`, slots). Missing dept mirror → throw/retry. |
| `WORKFORCE_EMPLOYMENT_HIRED` | Create Finance `Employee` (`cpEmploymentId`, `globalPersonId`); `salary: 0`. Ensures org; missing position → throw/retry. After create (and on idempotent re-delivery) **PATCH** CP `PATCH /internal/v1/workforce/employments/:id/finance-link` to set `financeEmployeeId`. |
| `WORKFORCE_EMPLOYMENT_TERMINATED` | **Consumed (wave 0):** set Finance `Employee.employmentStatus=TERMINATED` + `contractEndDate` for `(organizationId, cpEmploymentId)` only — does not touch a second VÖEN employment for the same person. Skip without `hr_full`; unknown employment → skip (no throw-loop); duplicate TERMINATED → idempotent. |
| `WORKFORCE_EMPLOYMENT_TRANSFERRED` | Update Finance `Employee.positionId`; resolve Employee by `financeEmployeeId` **or** `cpEmploymentId`; missing employee/position → throw/retry |

**Absence mirror (wave 0):** `WORKFORCE_ABSENCE_*` resolve Finance Employee by `financeEmployeeId` **or** `payload.employmentId` (`cpEmploymentId`). Missing employee → throw/retry (do not silent-skip — worker idempotency would bury vacations published before hire write-back).

**Holding / dual-VÖEN:** two STANDALONE orgs + Holding + two WorkforceScopes. Wave 0 Day-1 UX = org switcher. Federated holding HR UI (`CP-WF-GROUP-01`) = API + UI in wave 3 (not SHIPPED until UAT-SMOKE). Runbooks: [evrostar-wave-0.md](./runbooks/evrostar-wave-0.md), [evrostar-wave-3.md](./runbooks/evrostar-wave-3.md).
