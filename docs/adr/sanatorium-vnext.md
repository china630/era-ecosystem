...
| **SV12** | **Retail: контракты reserve + write-off.** `PRESCRIPTION_ISSUED` → request-reply «собрать корзину/забронировать препарат» (нужна проверка наличия сейчас). `PROCEDURE_COMPLETED` → списание расходников (по образцу `SATELLITE_FB_STOCK_CONSUMPTION_COMPLETED`) + начисление на folio/Accounting. |
