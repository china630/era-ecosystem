...
| **SV10** | **Lifecycle-события отеля на шину:** `SATELLITE_HOTEL_GUEST_CHECKED_IN`, `SATELLITE_HOTEL_GUEST_CHECKED_OUT`, `SATELLITE_HOTEL_ROOM_CHANGED`. Клиника реагирует переносом/отменой процедур при досрочном выезде/переселении. Walk-in close is **not** this event — reception + weekly cron; see [clinic-episode-as-clinical-course.md](./clinic-episode-as-clinical-course.md). |
