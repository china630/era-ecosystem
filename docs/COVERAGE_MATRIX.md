...
| CP-WF-GROUP-01 | Holding-federated HR (two VÖEN scopes, no merged tree) | ADR evrostar-workforce-pilot | Y | — | — | Y `/workspace/workforce/group` | — | API | directory `q` = FIN index + name (no UUID); EraListFilterBar; VIEWER alone 403; not SHIPPED (no UAT-SMOKE) |
...
|----|------------|-----|-----|-------|----------|----------|------------|-----------|
| CP-WF-EMP-01 | Minimal employment (MDM hire) | ADR cp-workforce-absence-split + provision-sync + role-templates | `POST /platform/v1/workforce/employments/hire`; `PATCH …/reprovision` `satelliteKeys`; list `q`/`sex`/`ageBucket` | — | — | Y | — | Hire + Login & access; server search (FIN blind index + name); PIN required when satellites; EraListFilterBar; Status=API — not SHIPPED |
