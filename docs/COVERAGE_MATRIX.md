...
| CLI-51 | PDF quota knots + nights interpolate + stay recalc | [ADR knots](./adr/nafta-program-quota-knots.md) | Y versioned templates + snapshot pin + one-current unique + GC; **W3** `resolveEntitlementCharge` + `listAmount` | Y `/admin/program-templates` blocks + retired toggle + purge; catalog missing-list filter | — | — | — | SCREEN | Not SHIPPED — UAT open; Wave B harden; **Blocker:** re-import Nafta prices so package rows gain `listAmount` before over-quota pilot |
