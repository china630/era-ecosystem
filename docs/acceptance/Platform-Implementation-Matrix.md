...
| AC-CP-WF | Workforce hub (hire, absence, seats, security) | ✅ | [ ] | `cp-wf-negative.spec.ts`; `workforce-timesheets.month.spec.ts`; `workforce-provision.reprovision.spec.ts`; `roster-cycle.util.spec.ts`; `workforce-roster.materialize.spec.ts`; `workforce-holding.spec.ts`; `workforce-personnel-orders.wave4.spec.ts`; `workforce-attendance.wave6.spec.ts`; `workforce-migration.service.spec.ts`; CP-WF-* | Negative: hire role deny 403; PLATFORM_WORKFORCE_REQUIRED 403. **Evrostar wave 3** federated HR (`CP-WF-GROUP-01` = API); **wave 4** order templates (`CP-WF-ORD-02` = API); **wave 6** attendance FaceID ingest (`CP-WF-ATT-01` = API, not SHIPPED); **CP-WF-MIG-01** migration wizard API+SCREEN (not SHIPPED) — [evrostar-wave-6.md](../runbooks/evrostar-wave-6.md) |
