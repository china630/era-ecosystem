...
export class ProfitTaxService {
   ...
   * amount = taxDepreciation тИТ bookDepreciation (per Wave 2 plan).
  ...
  async syncAutoTaxDepreciationAdjustment(organizationId: string, year: number) {
    ...
    const data = {
      ...
      description:
        "Tax vs book depreciation difference (tax тИТ book, NK Art. 114 declining balance)",
      amount,
    };
  ...
  private assertYear(year: number) {
    if (!Number.isFinite(year) || year < 2000 || year > 2100) {
      throw new BadRequestException("Invalid year (expected 2000тАУ2100)");
    }
  }
}
