...
- Ручные операции (**Journal Vouchers** / **əl ilə tənzimləmə**): экран **`/accounting/adjustments`**, API **`POST /api/accounting/manual-adjustments`**. Новая проведённая транзакция `Transaction.kind = MANUAL_ADJUSTMENT`; исходные документы не изменяются; поле **`reason` обязательно**. Шаблоны (возврат переплаты, списание 211/531, безвозмездное поступление) только подставляют счета. **Wave 2:** внутренняя **кредит-корректировка остатка инвойса** (`POST …/invoices/:id/credit-adjustment`, `InvoicePayment.kind=CREDIT_ADJUSTMENT`, без PDF-кредит-ноты); **капитализация ОС «дар»** (`creditSource=DONATION`); ссылки `basisInvoiceId` / `basisFixedAssetId` в реестре. **Wave 3:** при REVENUE credit-adjust **601/545/211** согласованы с НДС SENT; сторно своей корректировки (`reversesTransactionId`); справка-PDF; preview/copy/reverse; навигация основание↔документ. ADR: [finance-manual-adjusting-journal.md](../docs/adr/finance-manual-adjusting-journal.md). Автоматические проводки — от других модулей.
...
- **Кредит-корректировка остатка (wave 2–3):** уменьшение `remaining` через `InvoicePayment` + новая проводка; строки счёта и проведённый SENT не редактируются; PDF клиенту не выпускается. **Wave 3:** при `offset=REVENUE` и НДС — пропорциональный split **601 / 545 / 211** согласован с SENT; переплата после PAID — шаблон **AR_OVERCOLLECTION_REFUND** на `/accounting/adjustments`.
...
| **Инкассация** | Отдельный тип **MXO** для передачи наличных в банк (корреспонденция с `MAIN_BANK` / Q-01 **223** и т.п. по ТЗ). |
...
### 4.17. Finance Core — UI/UX и онбординг доработки (v-next, PLANNED)

**Статус блока:** [ ] **PLANNED (finance-core-ux-wave-1)** — пакет доработок ядра по UX-ревью локального стенда (`localhost:3100`). Цель — выровнять оболочку и ключевые формы ядра со стандартами спутников и устранить функциональные пробелы. Техконтракт и алгоритмы — [TZ.md](./TZ.md) **§27**. Все пункты ниже — **next version**, в текущей редакции не реализуются (кроме помеченных «уже в коде»).

| Feature ID | Область | Что делаем | Статус |
|------------|---------|------------|--------|
| **FEAT-FC-UX-001** | Шапка приложения | Единая шапка как на всех спутниках: цветовой прогресс-бар по текущему тиру (с использованием квот), панель выбора языка, панель выбора компании, иконка оповещений, иконка профиля | [~] PARTIAL |
| **FEAT-FC-UX-002** | Онбординг / выбор компании | Обязательный модальный выбор компании при логине, если в профиле доступно несколько организаций; модалку нельзя закрыть, пока компания не выбрана; состояние сохраняется при F5 | [ ] PLANNED |
| **FEAT-FC-UX-003** | `/banking` | Растянуть страницу на всю ширину контентной области | [ ] PLANNED |
| **FEAT-FC-UX-004** | `/banking/cash` | Растянуть страницу на всю ширину контентной области | [ ] PLANNED |
| **FEAT-FC-UX-005** | `/treasury/cash-flow` | Фильтр (Üfüq / gün / выбор дней) разместить в одну строку как единый блок фильтра | [ ] PLANNED |
| **FEAT-FC-UX-006** | `/finance/prepaid-expenses` | Перевести список на стандартную таблицу ERA; форму создания — в модалку; добавить поле описания/назначения расхода | [ ] PLANNED |
| **FEAT-FC-UX-007** | CRM: модалка «Yeni kontragent» | Сузить модалку на ~30%; пошаговый ввод от VÖEN; автозаполнение из реестра налоговой и нашей БД; дополнительные поля руководителя и контактов | [ ] PLANNED |
| **FEAT-FC-UX-008** | Продажи: «Yeni hesab» → «Ödənişdə Dt» | Убрать хардкод счетов 101/221; выбор денежного счёта зависит от вида организации (COMMERCIAL/BUDGET/NGO); при банке — выбор конкретного банковского счёта; привязка валюты к счёту | [ ] PLANNED |
| **FEAT-FC-UX-009** | Закупки: «Yeni alış fakturası» | Для ядра при создании счёта **не** запрашивать счёт оплаты (касса/банк) — оплата отдельным событием | [x] РЕШЕНО (продуктово) |

#### 4.17.1. Шапка приложения (FEAT-FC-UX-001)

Шапка ядра уже построена на общем ките спутников **`@era/satellite-kit/ui`** (`EraAppHeader`): присутствуют прогресс-бар тира (`HeaderTierUsageBar` через `HeaderSubscriptionStrip`), переключатель языка (`LanguageSwitcher`), переключатель организации (`HeaderOrganizationSwitcher`), колокольчик оповещений (`InAppNotificationBell`), меню профиля (`HeaderProfileMenu`). **Доработка v-next:** убедиться, что прогресс-бар тира имеет **цветовую** градацию по тиру (а не только числовые квоты), и что набор и порядок элементов идентичен референсному спутнику. Если продакшен отстаёт — выкатить актуальную сборку. Подробности — TZ §27.1.

#### 4.17.2. Обязательный выбор компании при логине (FEAT-FC-UX-002)

**Продуктовое правило:** пользователь без выбранной активной организации не должен иметь доступа к рабочему контуру. Если в профиле **несколько** организаций и активная не установлена — открывается **блокирующая модалка** выбора компании:

- модалку **нельзя** закрыть (нет крестика / Esc / клика по фону / кнопки «Отмена») до выбора компании;
- после F5 при отсутствии активной организации модалка открывается снова (детерминированно по состоянию сессии, а не по флагу в браузере);
- при **2+** организациях выбор **обязателен при каждом входе** — «последнюю активную» компанию авто-восстанавливать **не** будем;
- если организация **ровно одна** — она выбирается автоматически без модалки;
- если организаций **ноль** — действует текущий сценарий пустого рабочего пространства (`/companies`, создание/присоединение);
- super-admin сохраняет доступ к платформенному контуру (`/admin/data`).

Форма выбора компании на `/companies` остаётся отдельной страницей для управления (создание, холдинги, join по VÖEN); модалка онбординга — это самостоятельный обязательный gate. Техника — TZ §27.2.

#### 4.17.3. Полноширинные страницы банка и кассы (FEAT-FC-UX-003 / 004)

`/banking` и `/banking/cash` растянуть на всю ширину контентной области (`max-w-none`, как уже сделано на `prepaid-expenses`), убрав ограничители ширины. Это согласуется со стандартом реестров (широкие таблицы выписок/ордеров).

#### 4.17.4. Однострочный фильтр в Cash Flow (FEAT-FC-UX-005)

На `/treasury/cash-flow` контролы прогноза (**Üfüq**, **gün**, выбор количества дней) — это единый фильтр; разместить их в одну строку (как тулбар `PageHeader.actions`, выравнивание по высоте 32px), а не разрозненными блоками.

#### 4.17.5. Prepaid Expenses: таблица + модалка + описание (FEAT-FC-UX-006)

- Список расходов будущих периодов перевести с карточек-списка на **стандартную таблицу ERA** (`DATA_TABLE_*`), с пагинацией по стандарту реестров.
- Создание перевести в **модалку** (паттерн Create-модалок ядра), форма ввода (сумма, период, счета) — внутри модалки.
- Добавить поле **описания/назначения** расхода (например «Страховка ОСАГО 2026», «Аренда офиса Q1») — для читаемости реестра и проводок.

**Ответ на вопрос «связаны ли эти операции с проводками?» — ДА.** Создание расхода будущих периодов проводок **не** порождает: оно лишь строит график помесячной амортизации (`PrepaidExpenseSchedule`). Проводки формируются при **помесячном признании** (кнопка «постинг месяца», `POST /api/prepaid-expenses/:id/post-month`): на каждый период создаётся журнальная проводка **Дт расходный счёт (по умолчанию 731) — Кт счёт расходов будущих периодов (по умолчанию 133)** через `AccountingService.postJournalInTransaction` (NAS-леджер). Когда все периоды проведены — статус расхода переходит в `FULLY_AMORTIZED`. См. TZ §27.5.

#### 4.17.6. Контрагенты: модалка «Yeni kontragent» и VÖEN-workflow (FEAT-FC-UX-007)

**Изменения формы:**

- ширину модалки уменьшить на ~30% (узкая колоночная форма вместо `max-w-3xl`);
- **первая строка — VÖEN (10 rəqəm)** + кнопка **«Yoxla»**; остальные поля **пассивны (заблокированы)** до проверки;
- чекбокс **«ƏDV ödəyicisidir»** разместить в одной строке с полем **«Hüquqi təşkilat forması»**;
- до поля **«Ünvan»** добавить поля **«Rəhbərin A.S.A.»** (ФИО руководителя) и **«Əlaqə nömrələri»** (контактные номера — **динамический список**, мин. 1, с кнопкой «+ добавить»).

**Алгоритм по нажатию «Yoxla» (продуктовый):**

1. **Налоговая, юр. лицо:** запрос по `taxpayer-info` с фильтрами «Vergi ödəyicisinin növü = Hüquqi şəxs», «Axtarış növü = VÖEN». Берём: название, VÖEN, признак рискованности, статус ƏDV, активность, долг.
2. Если найден и **«ƏDV qeydiyyatındadır»** — дополнительно `search-vat-payer` по VÖEN: орган учёта, ОПФ, юр. адрес, уставный капитал, фин. год, законный представитель, дата госрегистрации, дата последнего изменения, долг.
3. Если по юр. лицу **не найден** — повтор с фильтрами «Vergi ödəyicisinin növü = Fizik şəxs». Берём данные физлица (ФИО, VÖEN, рискованность, ƏDV, активность, долг).
4. Если ни одним способом не нашли в базе налоговой — оставить флаг **«manual check (tax)»**.
5. Затем поиск по **нашей БД компаний** (data-hub). Если не нашли — флаг **«manual check (internal)»**.
6. Найденные данные подставляются в поля; чего нет — поля остаются пустыми.
7. После проверки **все поля разблокируются** для ручного редактирования.

**На будущее (зафиксировано, отдельные итерации):**

- банковские реквизиты контрагента — на **отдельной модалке**, открываемой из таблицы контрагентов; если реквизиты уже есть — открывается список с возможностью дополнения (база уже есть: `CounterpartyBankAccountsModal`);
- для **физических лиц** — поле **FIN-код** (связь с БД граждан — отдельная БД);
- **коды видов деятельности** юр./физ. лиц — получаем через наш **браузерный виджет** для отправки отчётов (RPA-расширение).

Техконтракт, текущее состояние интеграции и зоны ответственности (data-hub компании, БД граждан) — TZ §27.6.

#### 4.17.7. Продажи «Ödənişdə Dt»: счета по виду организации, банковский счёт, валюта (FEAT-FC-UX-008)

В модалке **«Yeni hesab»** поле **«Ödənişdə Dt»** сейчас хардкодит **101 Kassa** / **221 Bank**. Продуктовые решения:

- **Не хардкодить** счета: список денежных счетов формируется по **виду организации** (`OrganizationKind`: COMMERCIAL / BUDGET / NGO) из плана счётов NAS (NAS-COMMERCIAL / NAS-GOV / NAS-NGO), т.к. кассовые/банковские счета у гос и НГО отличаются.
- При выборе **банка** — добавить **второй селектор конкретного банковского счёта** организации (у орга может быть несколько счетов; список активных `BankAccount`).
- **Валюта привязана к счёту:** для банковского счёта валюта берётся из выбранного `BankAccount` (в т.ч. валютные счета); для кассы (**101**) допустим только **AZN**. Поле валюты автоматически синхронизируется с типом/счётом и блокируется, когда выбор однозначен.

См. TZ §27.7 (ответ на вопрос «где принимается решение, с какого счёта оплата» — на этом же шаге, через явный выбор банковского счёта).

#### 4.17.8. Закупки «Yeni alış fakturası»: счёт оплаты не нужен на этапе счёта (FEAT-FC-UX-009)

**Ответ на вопрос.** В модели двойной записи **счёт-фактура закупки — это начисление обязательства**, а не платёж: при проведении формируется задолженность перед поставщиком (**Кт 531**) и приход ТМЦ/расхода (**Дт 201/2xx/7xx**). Поэтому при создании «Yeni alış fakturası» в ядре **не нужно** указывать, на какой счёт принимать/с какого платить деньги.

**Оплата — отдельное событие** (кассовый расходный ордер MXO или банковский платёж), где и выбирается касса/банк/конкретный счёт по той же логике, что и в §4.17.7. Это даёт корректные частичные оплаты, отсрочки и взаимозачёты. В форме закупки оставляем только условия оплаты (срок `dueDate`); кнопка **«Оплатить»** доступна **и из реестра закупок** (по документу), **и из раздела кредиторки/531** (по контрагенту, с распределением) — единый эндпоинт платежа, две точки входа. TZ §27.8.

Техконтракт всего блока — [TZ.md](./TZ.md) §27.
...
#### 4.18.1. Справочники из Data Hub — что переносим на потребление
...
#### 4.18.2. БД граждан Азербайджана — отдельная БД на оркестраторе
...
**Правило (продуктовое и техническое):** ни один код счёта НР АР **не должен** встречаться строковым/числовым литералом в бизнес- и проводочной логике ядра. Источник кода — **только** `PostingAccountResolver.resolveAccountCode(orgId, role)` по ключу `PostingRole` (с учётом `OrganizationKind` и per-org override `organization_posting_roles`). Это критично для гос/НГО (NAS-GOV / NAS-NGO), где коды отличаются от коммерческих.

**Аудит (2026-06) выявил хардкоды** (полный список и фиксы — TZ §28.3). Главные:

- **Prepaid** (`731`/`133`) — дефолты и проводки амортизации (см. §4.17.5);
- **Инвойсы / PSA / satellite-dispatch** (`101`/`221`) — счёт зачисления по умолчанию (см. §4.17.7–4.17.8);
- **Manufacturing overhead** (`741`/`204`), **Inventory** дискриминаторы (`201`/`204`), **migration opening balances** (`201`/`204`/`000`), **organization-settings** (`221`), **accounts/bank-subaccount** (`221`/`211`/`601`), **banking/kassa** (`101`/`221`/`301`), **reports** (агрегация по префиксам).

**Недостающие ключи `PostingRole` (добавить):** `PREPAID_ASSET` (комм. `133`), `CHARTER_CAPITAL` (комм. `821`); банковские счета Q-01 — `MAIN_BANK`/`BANK_SETTLEMENT`→`223`, прочие до востребования `224` (`VAT_DEPOSIT_ACCOUNT`). Код `538` используется лишь как «анти-касса» guard — оставить как валидатор, не как роль. Код `000` (контра для остатков) — системный, документируется отдельно.
...
Техконтракт, таблица фиксов и порядок волн — [TZ.md](./TZ.md) §28.

---

### 4.19. Внутрисетевой обмен документами и зеркальные проводки (v-next, PLANNED)

**Статус блока:** [ ] **PLANNED (intercompany-network-wave-1)** — сетевой эффект экосистемы: когда **обе стороны** сделки — резиденты ERA (в финядре или сателлите, неважно), один экономический факт у продавца порождает **входящий документ** у покупателя и (после подтверждения) **зеркальную проводку** через posting-роли. Аналог внутреннего e-invoicing: ускоряет ввод первички, исключает двойной ручной ввод, даёт достоверную взаимную сверку и неттинг. Техконтракт — [TZ.md](./TZ.md) **§29**.

#### 4.19.1. Зачем это реализуемо у нас (готовый фундамент)

| Готовое звено | Где | Роль в фиче |
|---------------|-----|-------------|
| Идентификация контрагент→тенант по VÖEN | `Organization.taxIdBlindIndex` (`@unique`), `Counterparty.taxIdBlindIndex` / `globalId`, `GlobalCompanyDirectory` | Понять, что контрагент B — тоже тенант системы |
| Движок проводок по ключам | `PostingAccountResolver` + posting-роли (§4.12, §4.18) | Собрать корректное зеркало с учётом `OrganizationKind` (комм./гос/НГО) |
| Событийный постинг | `SatelliteEventDispatchService` | Тот же конвейер: документ → проводка |
| Сверка и неттинг | `counterparty-reconciliation-build.ts`, posting-схема `NETTING` | Взаимозачёт двух «своих» сторон с двумя авторитетными книгами |

#### 4.19.2. Модель (зафиксированные решения)

- **FEAT-FC-NET-001 — Матчинг сторон.** При создании исходящего счёта продавцом A контрагент сопоставляется с реальным тенантом B по blind-index VÖEN.
- **FEAT-FC-NET-002 — Входящий ящик + подтверждение (D-NET-1).** У B документ появляется как **входящий черновик** (`PENDING_REVIEW`). **Молчаливого автопостинга в чужую книгу нет.** B принимает и **сам выбирает роль дебета** (товар `INVENTORY_GOODS` / расход `MISC_OPERATING_EXPENSE` / ОС / РБП), решает брать ли НДС к зачёту и период. После подтверждения — зеркальная проводка авто-генерится по posting-ролям.
- **FEAT-FC-NET-003 — Согласие (opt-in, D-NET-3).** Приём сетевых документов включается настройкой организации B. Передаётся только документ, адресованный самому B (приватность не нарушается).
- **FEAT-FC-NET-004 — Eventual consistency (D-NET-4).** A и B — разные тенанты (свои audit-цепочки, `OrganizationSecurityState`, закрытые периоды). Никакой единой кросс-тенантной транзакции: обмен через **outbox-событие**, идемпотентно по `correlationId`, со сверкой. У B уважаются period-lock, `HARD_BLOCK`, согласования.
- **FEAT-FC-NET-005 — Связь с e-Qaimə (D-NET-2).** Легальный источник B2B-счёта в АР — **e-Qaimə** (налоговый портал). Наш обмен **предзаполняет** e-Qaimə и служит внутренней сверкой; он **не подменяет** легальный поток.
- **FEAT-FC-NET-006 — Автонеттинг.** Для двух резидентов встречные AR/AP сводятся достоверно (обе книги авторитетны) через схему `NETTING`.

#### 4.19.3. Фазы

- **Фаза 1 (MVP):** матчинг VÖEN→тенант + входящий ящик (черновики) + 1 клик «Принять» с выбором роли дебета → зеркальная проводка. Opt-in настройка B.
- **Фаза 2:** straight-through автопостинг **за флагом** (только безопасные роли, по умолчанию расход; активы/запасы — всегда review) + автонеттинг встречных AR/AP.
- **Фаза 3:** маппинг на e-Qaimə (предзаполнение) и кросс-деплойный обмен через оркестратор (когда покупатель в другом деплое).

**Граница:** каждая сторона ведёт свою книгу; зеркало не идентично (роли дебета/НДС/период у B — его выбор). Per-tenant данные не смешиваются. Техконтракт, модель данных и потоки — [TZ.md](./TZ.md) §29.
