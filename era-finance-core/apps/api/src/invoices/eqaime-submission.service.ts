...
export class EqaimeSubmissionService {
  ...
  async submit(organizationId: string, invoiceId: string) {
    ...
    if (inv.isInternational) {
      throw new BadRequestException("International invoices cannot be submitted as e-Qaimə");
    }
    if (inv.currency !== "AZN") {
      throw new BadRequestException("Only AZN invoices support e-Qaimə S2S");
    }
    if (
      inv.eqaimeStatus === EqaimeStatus.SUBMITTED ||
      inv.eqaimeStatus === EqaimeStatus.ACCEPTED
    ) {
      throw new BadRequestException("e-Qaimə already submitted for this invoice");
    }
      ...
      throw new HttpException(
        {
          ...
          message:
            "Organization.settings.tax.asanUserId is required for e-Qaimə S2S. Configure in organization settings.",
        },
        HttpStatus.SERVICE_UNAVAILABLE,
      );
    }
  ...
  /**
   * Incoming pull stub from a submitted e-qaimə payload (no network doc).
   */
