...
const LINK_GROUPS: { groupKey: string; items: { href: string; labelKey: string }[] }[] = [
  {
    ...
    items: [{ href: "/admin/data/system-config", labelKey: "superAdmin.dataHubSystemConfig" }],
  },
  {
    groupKey: "superAdmin.dataHubNavCatalogs",
    items: [
      { href: "/admin/data/currencies", labelKey: "superAdmin.dataHubCurrencies" },
      { href: "/admin/data/units-of-measure", labelKey: "superAdmin.dataHubUom" },
      { href: "/admin/data/tax-rates", labelKey: "superAdmin.dataHubTaxRates" },
      { href: "/admin/data/customs-tariffs", labelKey: "superAdmin.dataHubCustoms" },
    ],
  },
  {
    groupKey: "superAdmin.dataHubNavNas",
    items: [
      { href: "/admin/data/nas-chart", labelKey: "superAdmin.dataHubNasChart" },
      { href: "/admin/data/nas-templates", labelKey: "superAdmin.dataHubNasTemplates" },
    ],
  },
  {
    ...
    <nav className="w-full lg:w-56 shrink-0 space-y-5 border-b lg:border-b-0 lg:border-r border-[#E5E9EC] pb-4 lg:pb-0 lg:pr-4">
      <Link
        ...
        className={[
          ...
          pathname === "/admin/data" ? activeClass : idleClass,
        ].join(" ")}
      >
      ...
      {LINK_GROUPS.map((g) => (
        <div key={g.groupKey}>
          ...
          <ul className="space-y-0.5">
            {g.items.map((it) => {
              const active =
                pathname === it.href || (it.href !== "/admin/data" && pathname.startsWith(it.href));
              return (
                <li key={it.href}>
                  <Link
                    href={it.href}
