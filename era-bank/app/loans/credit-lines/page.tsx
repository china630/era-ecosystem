...
export default function CreditLinesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/loans/credit-lines", { cache: "no-store" });
      if (!res.ok) {
  ...
  async function onCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/loans/credit-lines", {
        method: "POST",
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Credit lines"
        subtitle="Limits and drawdown approval queue"
        actions={
      ...
      <BankDataGrid
        ...
        columns={[
          ...
          },
          {
            key: "limitMinor",
            header: "Limit",
            render: (r: Row) => String(r.limitMinor ?? ""),
          },
          {
            key: "status",
            header: t("colStatus"),
      ...
      />
      <OpsModalShell open={mode === "create"} onClose={close} title="New credit line">
        <form className="space-y-3" onSubmit={onCreate}>
          ...
          <Field name="limitMinor" label="limitMinor" preset="code" required />
