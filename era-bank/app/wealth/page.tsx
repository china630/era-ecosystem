...
export default function Page() {
  const t = useTranslations("pages.wealth");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/wealth/safekeeping", { cache: "no-store" });
      if (!res.ok) {
  ...
  async function onCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/wealth/safekeeping", {
        method: "POST",
    ...
    <div className="space-y-4">
      ...
      <BankDataGrid
        ...
        columns={[
          ...
          {
            key: "accountNo",
            header: t("colAccountNo"),
            render: (r: Row) => String(r.accountNo ?? ""),
          },
          {
            key: "customerId",
          ...
          },
          {
            ...
            render: (r: Row) => String(r.currency ?? ""),
          }
        ]}
      />
      <OpsModalShell open={mode === "create"} onClose={close} title={t("createTitle")}>
        <form className="space-y-3" onSubmit={onCreate}>
          ...
          <Field name="accountNo" label={t("accountNo")} preset="code" required  />
