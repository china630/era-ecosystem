...
export default function Page() {
  const t = useTranslations("pages.cardDisputes");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/cards/disputes/list", { cache: "no-store" });
      if (!res.ok) {
  ...
  async function onCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/cards/disputes", {
        method: "POST",
    ...
    <div className="space-y-4">
      ...
      <BankDataGrid
        ...
        columns={[
          ...
          },
          {
            ...
            header: t("colCardTransactionId"),
            render: (r: Row) => String(r.cardTransactionId ?? ""),
          },
          {
            key: "amountMinor",
      ...
      <OpsModalShell open={mode === "create"} onClose={close} title={t("createTitle")}>
        <form className="space-y-3" onSubmit={onCreate}>
          <Field name="cardTransactionId" label={t("cardTransactionId")} preset="code" required  />
          <Field name="amountMinor" label={t("amountMinor")} preset="code" required  />
          <Field name="reasonCode" label={t("reasonCode")} preset="code" required  />
