...
export default function BankAccessPage() {
  ...
  const [cloneFrom, setCloneFrom] = useState("BRANCH_MANAGER");
  ...
  const userOptions = useMemo(
    () =>
      users.map((u) => ({
        value: u.id,
        label: `${u.fullName} (${u.username}) — ${u.roleCode}`,
      })),
    [users],
  );
  ...
  const loadRoles = useCallback(async () => {
    ...
    try {
      ...
      if (!assignRoleCode && rows[0]) setAssignRoleCode(rows[0].code);
    } catch (e) {
      showApiError({ error: e instanceof Error ? e.message : tc("loadError") });
    } finally {
      setLoading(false);
    }
  }, [assignRoleCode, cloneFrom, tc]);
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      setUsers(rows);
      if (!assignUserId && rows[0]) setAssignUserId(rows[0].id);
    } catch {
      // users list optional if admin:users missing
    }
  }, [assignUserId]);
