...
export default function Page() {
  const t = useTranslations("pages.amlCases");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/aml/cases", { cache: "no-store" });
      if (!res.ok) {
    ...
    <div className="space-y-4">
      ...
      <OpsModalShell open={mode === "create"} onClose={close} title={t("createTitle")}>
        <form className="space-y-3" onSubmit={onCreate}>
          <Field name="alertId" label={t("alertId")} preset="code"   />
          <Field name="customerId" label={t("customerId")} preset="code"   />
