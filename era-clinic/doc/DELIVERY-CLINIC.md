...
- [~] W2 block axes + auto-apply: `assignMode`/`fulfillment`/`quotaBasis`/`requiresDoctor` on template blocks; `applyPackageAutoBlocks` (OPEN/DAY1/CARE_TEAM/MANUAL_RETRY); `packageSignal` + confirm-no-package; intake blocks in `npm run db:seed` (`prisma/seed-intake-blocks.mjs`); admin matrix PER_STAY + auto badge; UI UAT deferred
