...
const content = `import type { ProcedureCheckInChannel, ProcedureOrder } from "@prisma/client";
...
export async function markProcedureNoShow(orderId: string, actor: AttendanceActor) {
  ...
  if (charge.shouldChargeFolio && order.reservationId) {
    await postProcedureFolioCharge({
      ...
      externalTicketId: \`clinic-noshow-\${order.id}\`,
    });
  }
...
const out = path.join(
  ...
  "domain",
  "procedure",
  "procedure-attendance.service.ts",
);
fs.writeFileSync(out, content, "utf8");
const b = fs.readFileSync(out);
console.log(b[1] === 0 ? "BAD utf16" : "utf8 ok", out);
