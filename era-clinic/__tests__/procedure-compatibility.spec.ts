...
describe('procedure compatibility (FORBID_SAME_DAY)', () => {
  it('blocks same-day pair', () => {
    ...
    const ok = checkForbidSameDay(
      ...
      [{ procedureCode: 'HEAT', startAt: day, endAt: day }],
  ...
  it('allows different days', () => {
    const ok = checkForbidSameDay(
      ...
      [{ procedureCode: 'HEAT', startAt: new Date('2026-07-01T10:00:00Z'), endAt: new Date('2026-07-01T11:00:00Z') }],
      { procedureCodeA: 'CRYO', procedureCodeB: 'HEAT' },
    );
    expect(ok).toBe(true);
  });
...
describe('rule types enum', () => {
  const types: ProcedureCompatibilityRuleType[] = [
    'FORBID_SAME_DAY',
    'MIN_HOURS_GAP',
    'FORBID_SEQUENCE',
  ];
