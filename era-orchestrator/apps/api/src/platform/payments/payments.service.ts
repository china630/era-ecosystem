...
export class PaymentsService {
  ...
  async reconcilePaymentLink(organizationId: string, token: string) {
    this.idempotency.assertLiveMode();
    await this.entitlement.assertPlatformModule(organizationId, ENTITLEMENT);
    const link = await this.prisma.platformPaymentLink.findFirst({
      where: { token, organizationId },
    });
    if (!link) throw new BadRequestException("Payment link not found");
    if (link.status === PlatformPaymentLinkStatus.PAID) {
      return { link, reconciled: false, alreadyPaid: true };
    }
    const updated = await this.prisma.platformPaymentLink.update({
      where: { id: link.id },
      data: { status: PlatformPaymentLinkStatus.PAID },
    });
    await this.audit.log({
      organizationId,
      addonSlug: ENTITLEMENT,
    ...
    return { link: updated, reconciled: true, mode: "live" };
  }
