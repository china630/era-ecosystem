...
export function buildPricingStorefrontView(
  ...
  const hospitalityBundles = [...(api.hospitalityBundles ?? [])]
    .filter((b) => b.discountedPriceAzn > 0)
    .sort((a, b) => b.discountedPriceAzn - a.discountedPriceAzn)
    .map((b) => {
      const meta = ui.bundles[b.marketingId];
  ...
  const hospitalityModules = (api.hospitalityModules ?? []).map((m) => ({
    key: m.key,
    name: ui.hospitalityModuleNames[m.key] ?? m.name,
  ...
  return {
    ...
    hospitalityTitle: ui.hospitalityTitle,
