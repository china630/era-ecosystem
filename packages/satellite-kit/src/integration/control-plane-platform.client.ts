...
async function platformGet<T>(path: string, opts?: PlatformCallOptions): Promise<T> {
  ...
  if (!res.ok) {
    throw new Error(`Control plane GET ${path} failed: ${res.status}`);
  }
  return json as T;
}
...
export function createBookingSlots(body: Record<string, unknown>, opts?: PlatformCallOptions) {
  return platformPost("/platform/booking/v1/slots", body, opts);
}

export function createBookingAppointment(body: Record<string, unknown>, opts?: PlatformCallOptions) {
  return platformPost("/platform/booking/v1/appointments", body, opts);
}
...
export function createPromotion(body: Record<string, unknown>, opts?: PlatformCallOptions) {
  return platformPost("/platform/loyalty/v1/promotions", body, opts);
}

export function createCustomDomain(body: Record<string, unknown>, opts?: PlatformCallOptions) {
  return platformPost("/platform/domains/v1/domains", body, opts);
}

export function createShipment(body: Record<string, unknown>, opts?: PlatformCallOptions) {
  return platformPost("/platform/delivery/v1/shipments", body, opts);
}

/** Billing consumer — org subscription snapshot (CP-BILLING). */
