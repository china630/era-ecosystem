 ...
 * Portable acceptance-kit (no product-specific paths).
...
const rules = [
  ...
  },
  {
    re: /\bgit\s+stash\b/i,
    permission: "ask",
    reason: "git stash can hide unrelated work; confirm with the user before stashing.",
  },
  {
    re: /\b(rm\s+-rf|remove-item\s+-recurse)\b.*(\.git\b|deploy[/\\].*prod|\.env\b)/i,
    permission: "deny",
